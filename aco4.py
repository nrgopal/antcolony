...
# True optimal cost = (1*1) + (16*16) + (16*16) + (16*16) = 
OPTIMAL_COST = 769
...
# Q is the constant factor used to increase pheromone
Q = 2
...
# The objective cost function: iso3dfd_time
...
def update_pheromone():
    ...
    if (DEPO_OPTION == 0): # deposit for best ant only
        pheromone[best_thread, \
                  best_cache1, \
                  best_cache2, \
                  best_cache3] += Q*worst_cost/best_cost
    if (DEPO_OPTION == 1): # random deposits for all ants
        for i in range(NANTS):
            pheromone[selected_thread[i], \
                      selected_cache1[i], \
                      selected_cache2[i], \
                      selected_cache3[i]] += Qworst_cost/random.randint(0,worst_cost)
    if (DEPO_OPTION == 2): # all ants based on cost
        for i in range(NANTS):
            pheromone[selected_thread[i], \
                      selected_cache1[i], \
                      selected_cache2[i], \
                      selected_cache3[i]] += Q*worst_cost/costs[i]
    ...
    if best_thread > 0:
        ...
        if neighbor_cost < best_cost:
           pheromone[best_thread-1, \
                     best_cache1,   \
                     best_cache2,   \
                     best_cache3] += Q*worst_cost/neighbor_cost
    ...
    if best_cache3 > 0:
        ...
        if neighbor_cost < best_cost:
            pheromone[best_thread, \
                      best_cache1, \
                      best_cache2, \
                      best_cache3-1] += Q*worst_cost/neighbor_cost
